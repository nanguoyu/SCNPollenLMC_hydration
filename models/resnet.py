...
class HHN_ResNet18Partial(nn.Module):
    def __init__(self, D, hin, num_classes=10):
        super(HHN_ResNet18Partial, self).__init__()
        ...
        # Pre-Conv
        self.conv1_weight_list = self.create_param_combination_conv2d_nod(3, 64, kernel_size=3)
        self.bn1 = nn.BatchNorm2d(64)
        ...
        # Layer 2
        self.conv2_1_1_weight_list = self.create_param_combination_conv2d_nod(64, 128, kernel_size=3)
        self.bn2_1_1 = nn.BatchNorm2d(128)
        self.conv2_1_2_weight_list = self.create_param_combination_conv2d_nod(128, 128, kernel_size=3)
        self.bn2_1_2 = nn.BatchNorm2d(128)
        self.shortcut2_1_weight_list = self.create_param_combination_conv2d_nod(64, 128, kernel_size=1)
        self.shortcut2_2 = nn.BatchNorm2d(128)

        self.conv2_2_1_weight_list = self.create_param_combination_conv2d_nod(128, 128, kernel_size=3)
        self.bn2_2_1 = nn.BatchNorm2d(128)
        self.conv2_2_2_weight_list = self.create_param_combination_conv2d_nod(128, 128, kernel_size=3)
        self.bn2_2_2 = nn.BatchNorm2d(128)

        # Layer 3
        self.conv3_1_1_weight_list = self.create_param_combination_conv2d_nod(128, 256, kernel_size=3)
        self.bn3_1_1 = nn.BatchNorm2d(256)
        self.conv3_1_2_weight_list = self.create_param_combination_conv2d_nod(256, 256, kernel_size=3)
        self.bn3_1_2 = nn.BatchNorm2d(256)
        self.shortcut3_1_weight_list = self.create_param_combination_conv2d_nod(128, 256, kernel_size=1)
        self.shortcut3_2 = nn.BatchNorm2d(256)

        self.conv3_2_1_weight_list = self.create_param_combination_conv2d_nod(256, 256, kernel_size=3)
        self.bn3_2_1 = nn.BatchNorm2d(256)
        self.conv3_2_2_weight_list = self.create_param_combination_conv2d_nod(256, 256, kernel_size=3)
        self.bn3_2_2 = nn.BatchNorm2d(256)

        # Layer 4
        self.conv4_1_1_weight_list = self.create_param_combination_conv2d_nod(256, 512, kernel_size=3)
        self.bn4_1_1 = nn.BatchNorm2d(512)
        self.conv4_1_2_weight_list = self.create_param_combination_conv2d_nod(512, 512, kernel_size=3)
        self.bn4_1_2 = nn.BatchNorm2d(512)
        self.shortcut4_1_weight_list = self.create_param_combination_conv2d_nod(256, 512, kernel_size=1)
        self.shortcut4_2 = nn.BatchNorm2d(512)

        self.conv4_2_1_weight_list = self.create_param_combination_conv2d_nod(512, 512, kernel_size=3)
        self.bn4_2_1 = nn.BatchNorm2d(512)
        self.conv4_2_2_weight_list = self.create_param_combination_conv2d_nod(512, 512, kernel_size=3)
        self.bn4_2_2 = nn.BatchNorm2d(512)

        # Output Layer
        self.linear_weight_list, self.linear_bias_list = self.create_param_combination_linear_nod(512, 10)
    ...
    def create_param_combination_conv2d_nod(self, in_channels, out_channels, kernel_size=3):
        weight_list = nn.ParameterList()
        weight = Parameter(torch.empty((out_channels, in_channels, kernel_size, kernel_size)))
        init.kaiming_uniform_(weight, a=math.sqrt(5))
        weight_list.append(weight)
    ...
    def create_param_combination_linear_nod(self, in_features, out_features):
        weight_list = nn.ParameterList()
        bias_list = nn.ParameterList()
        weight = Parameter(torch.empty((out_features, in_features)))
        init.kaiming_uniform_(weight, a=math.sqrt(5))
    ...
    def forward(self, x, hyper_x):
        ...
        # Pre-Conv
        x = F.conv2d(x, weight=self.conv1_weight_list[0], stride=1, padding=1)
        x = self.bn1(x)
        x = F.relu(x)
        ...
        # Layer 2
        res = F.conv2d(x, weight=self.conv2_1_1_weight_list[0], stride=2, padding=1)
        res = self.bn2_1_1(res)
        res = F.relu(res)
        res = F.conv2d(res, weight=self.conv2_1_2_weight_list[0], stride=1, padding=1)
        res = self.bn2_1_2(res)
        x = F.conv2d(x, weight=self.shortcut2_1_weight_list[0], stride=2)
        x = self.shortcut2_2(x)
        x = res + x
        x = F.relu(x)

        res = F.conv2d(x, weight=self.conv2_2_1_weight_list[0], stride=1, padding=1)
        res = self.bn2_2_1(res)
        res = F.relu(res)
        res = F.conv2d(res, weight=self.conv2_2_2_weight_list[0], stride=1, padding=1)
        res = self.bn2_2_2(res)
        x = res + x
        x = F.relu(x)

        # Layer 3
        res = F.conv2d(x, weight=self.conv3_1_1_weight_list[0], stride=2, padding=1)
        res = self.bn3_1_1(res)
        res = F.relu(res)
        res = F.conv2d(res, weight=self.conv3_1_2_weight_list[0], stride=1, padding=1)
        res = self.bn3_1_2(res)
        x = F.conv2d(x, weight=self.shortcut3_1_weight_list[0], stride=2)
        x = self.shortcut3_2(x)
        x = res + x
        x = F.relu(x)

        res = F.conv2d(x, weight=self.conv3_2_1_weight_list[0], stride=1, padding=1)
        res = self.bn3_2_1(res)
        res = F.relu(res)
        res = F.conv2d(res, weight=self.conv3_2_2_weight_list[0], stride=1, padding=1)
        res = self.bn3_2_2(res)
        x = res + x
        x = F.relu(x)

        # Layer 4
        res = F.conv2d(x, self.conv4_1_1_weight_list[0], stride=2, padding=1)
        res = self.bn4_1_1(res)
        res = F.relu(res)
        res = F.conv2d(res, self.conv4_1_2_weight_list[0], stride=1, padding=1)
        res = self.bn4_1_2(res)
        x = F.conv2d(x, self.shortcut4_1_weight_list[0], stride=2)
        x = self.shortcut4_2(x)
        x = res + x
        x = F.relu(x)

        res = F.conv2d(x, self.conv4_2_1_weight_list[0], stride=1, padding=1)
        res = self.bn4_2_1(res)
        res = F.relu(res)
        res = F.conv2d(res, self.conv4_2_2_weight_list[0], stride=1, padding=1)
        res = self.bn4_2_2(res)
        x = res + x
        x = F.relu(x)
        ...
        #x = self.execute_hyper_linear(x, self.linear_weight_list, self.linear_bias_list)
        x = F.linear(x, self.linear_weight_list[0], self.linear_bias_list[0])
