    ...
    for l in layers:
        for w in widths:
            ...
            for i, d in zip(range(len(dimensions)), dimensions):
                file_name = f'{output}/output/{transform}/SCN/scn_{arch}_{l}_{w}_{d}/acc.npy'
...
def dacc(arch, widths, layers):
    ...
    for l in layers:
        for w in widths:
            ...
            # One4All2xwider
            file_name = f'{output}/output/{transform}/One4All/{arch}2xwider_{l}_{w}/acc.npy'
            acc_one4all2x = pickle.loads(np.load(file_name))
            print(f"One4All2XWider acc={np.mean(acc_one4all2x)}")

            # One4All4xWider
            file_name = f'{output}/output/{transform}/One4All/{arch}4xwider_{l}_{w}/acc.npy'
            acc_one4all4x = pickle.loads(np.load(file_name))
            print(f"One4All4XWider acc={np.mean(acc_one4all4x)}")

            # One4All8xWider
            # file_name = f'{output}/output/{transform}/One4All/{arch}8xwider_{l}_{w}/acc.npy'
            # acc_one4all8x = pickle.loads(np.load(file_name))
            # print(f"One4All8XWider acc={np.mean(acc_one4all8x)}")
            ...
            add_label(ax.violinplot(acc_one4all, [-4], widths=1), 'One4All')
            add_label(ax.violinplot(acc_one4all2x, [-3], widths=1), 'One4All2XWider')
            add_label(ax.violinplot(acc_one4all4x, [-2], widths=1), 'One4All4XWider')
            # add_label(ax.violinplot(acc_one4all8x, [-1], widths=1), 'One4All8XWider')
            ...
            plt.legend(*zip(*labels), loc='upper right', prop={'size': 10}, ncol=2)
            ...
            plt.savefig(f"{output}/figs/{transform}/d_{arch}_{l}_{w}.png", bbox_inches='tight', dpi=200)
...
if __name__ == '__main__':
    ...
    viz('lenet', widths=[32], layers=[2], dimensions=[1, 2, 3, 5, 8])
    dacc('lenet', widths=[32], layers=[2])
